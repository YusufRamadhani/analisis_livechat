...
def insert_newimportantword(conn, values_insert):
    query = "INSERT INTO important_word (word, main_word, is_usage) VALUES " + \
        values_insert
...
def split_Words(**kwargs):
    ...
    words = task_instance.xcom_pull(
        task_ids='stemLive_Chat', key='wordsLiveChat')
...
def check_StoredImportantWord(**kwargs):
    task_instance = kwargs['task_instance']
    stored_word = task_instance.xcom_pull(
        task_ids='getImportant_Word', key='importantWord')
    stored_word = stored_word.values.tolist()

    words_split = task_instance.xcom_pull(
        task_ids='split_Words', key='splitWord')
...
def getDataStopWord(**kwargs):
    task_instance = kwargs['task_instance']
    result = get_stopword(connHiveJDBC)
    result = pd.DataFrame(result)
...
def getWordImportantWord(**kwargs):
    task_instance = kwargs['task_instance']
    result = get_importantwords(connHiveJDBC)
    result = pd.DataFrame(result)
...
def insertNewImportantWord(**kwargs):
    task_instance = kwargs['task_instance']
    values_insert = task_instance.xcom_pull(task_ids='check_StoredImportantWord',
                                            key='insertValue')
...
[taskGetImportantWord,
    taskSplitWords] >> taskCheckStoredImportantWord >> taskInsertNewImportantWord
