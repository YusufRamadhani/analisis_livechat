...
# Initiate DAGs
dag = DAG(
    'Analisis_Livechat-Hive-v1.0-Test',
...
# Create Connection JDBC. the 'HiveJDBC' is stored in airflow => admin => connection
connHiveJDBC = JdbcHook(jdbc_conn_id='HiveJDBC')


# fetching to table for query 'select'. so its for record data table
...
# query to create tmp table. temporary table for new data table livechat_bigram, table property 'transactional' = true and
# orc format file for 'transactional-property' = default
def create_tmpTable(conn):
...
# query insert new data for livechatbigram
def insert_tmpTable(values, conn):
    query = "INSERT INTO temp_table (id, loglivechatid, tipe, percakapan, `date`, percakapan_modify) VALUES " + values
    return getConnection(query, conn)


# update for matched row and insert for new data (update process)
def merge_tables(conn):
    query = "MERGE into bigram_livechat trg " \
...
def stemDataLiveChat(**kwargs):
    ...
    data = task_instance.xcom_pull(
        task_ids='getLiveChat_Message', key='dataLiveChat')
...
def bigramWords(**kwargs):
    task_instance = kwargs['task_instance']
    words = task_instance.xcom_pull(
        task_ids='stemDataLive_Chat', key='stemWords')
    words = words.values.tolist()
    data = task_instance.xcom_pull(
        task_ids='getLiveChat_Message', key='dataLiveChat')
    data = data.values.tolist()
    listwords = task_instance.xcom_pull(
        task_ids='getImportant_Word', key='listImportantWord')
...
def synonymMainWord(**kwargs):
    task_instance = kwargs['task_instance']
    list_main = task_instance.xcom_pull(
        task_ids='getImportantMain_Word', key='listMainImportantWord')
...
def getDataLivechatMessage(**kwargs):
    ...
    result = get_livechatmessage(startdate, today, connHiveJDBC)
...
def getMainImportantWord(**kwargs):
    task_instance = kwargs['task_instance']
    result = get_importantword_main(connHiveJDBC)
    result = pd.DataFrame(result)
...
####
def createTmpTable():
    return create_tmpTable(connHiveJDBC)
...
def storeNewData(**kwargs):
    ...
    value = task_instance.xcom_pull(
        task_ids='synonymMain_Word', key='newDataLiveChat')
    ...
    conn = connHiveJDBC
    insert_tmpTable(values_update, conn)
    merge_tables(conn)


def dropTmpTable():
    return drop_table(connHiveJDBC)
...
taskCreateTmpTable = PythonOperator(
    task_id='createTmp_Table',
    python_callable=createTmpTable,
    dag=dag
)
...
taskStart >> [taskGetLiveChatMessage,
              taskGetImportantWord, taskGetImportantMainWord]
taskGetLiveChatMessage >> taskStemDataLiveChat
[taskStemDataLiveChat, taskGetImportantWord] >> taskBigramWords
[taskGetImportantMainWord, taskBigramWords] >> taskSynonymMainWord >> taskCreateTmpTable >> taskStoreNewData >> taskDropTable >> taskFinish
