...
stoplist = [x[0] for x in sw.values.tolist()] # stoplist is list [0] in sw value
...
def insert_important_word():
    ...
    stored_word = pd.read_sql_query("SELECT word FROM important_word", conn).values.tolist()
    ##################################################################
    ...
    ############################################################################
        query_insert = "INSERT INTO important_word (word, main_word, is_usage) VALUES " + values_insert
